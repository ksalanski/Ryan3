...
function drawResponsiveSummary() {
  ...
  textStyle(NORMAL);
  textSize(textSizeValue);
  text("Lot podstawowy: 420 PLN", width * 0.05, y);
  total += 420;
  y += lineHeight;
  
  if (selectedSeat !== null) {
    text("Wybór miejsca: 69 PLN", width * 0.05, y);
    total += 69;
    y += lineHeight;
  }
  ...
  text("Opłata za oddychanie: 21,37 PLN", width * 0.05, y);
  total += 21.37;
  y += lineHeight;
  ...
  // Przycisk powrotu (left side)
  fill(255, 209, 0);
  drawRoundRect(width * 0.05, buttonY, buttonWidth, buttonHeight, 10);
  fill(7, 53, 144);
  textAlign(CENTER);
  textStyle(BOLD);
  textSize(constrain(width * 0.016, 12, 20));
  text("Powrót", width * 0.05 + buttonWidth/2, buttonY + buttonHeight/2 + 5);
  
  // Przycisk "Leć dalej!" (right side)
  let continueButtonX = width - buttonWidth - width * 0.05;
  fill(255, 209, 0);
  drawRoundRect(continueButtonX, buttonY, buttonWidth, buttonHeight, 10);
  fill(7, 53, 144);
  textAlign(CENTER);
  textStyle(BOLD);
  textSize(constrain(width * 0.016, 12, 20));
...
function mousePressed() {
  ...
  if (!showSummary && !showFinalScreen) {
    ...
    // Przycisk powrotu (left)
    if (mouseX > width * 0.05 && mouseX < width * 0.05 + buttonWidth && 
        mouseY > buttonY && mouseY < buttonY + buttonHeight) {
      showSummary = false;
    }
    
    // Przycisk "Leć dalej!" (right)
    let continueButtonX = width - buttonWidth - width * 0.05;
